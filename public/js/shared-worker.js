(function () {
	...
	//-------------------------------------------------------------------------
	// Shared Web Worker Setup
	//-------------------------------------------------------------------------
	
	if (!window.SharedWorker) {
		var msg = 'This browser does not support the shared web worker API.';
		log(msg); alert(msg); throw new Error(msg);
	}
	
	var sharedWorker = new window.SharedWorker('/js/shared-worker-impl.js', 'shared-worker-impl');
	...
	function onMessage(data) {
		switch (data.message) {
			...
			case 'meta-ping':
				postMessage('meta-pong', { payload: data.payload });
				break;
				
			case 'meta-ping-result':
				log('Ping-pong with ' + testLog.payload.size + ' bytes to ' + testLog.windowCount + ' window' + 
					(testLog.windowCount > 1 ? 's' : '') + ' ' + testLog.times + ' times took ' + data.millisecs + ' milliseconds.');
				onTestFinished(data.millisecs);
				break;
		}
	}
	...
	function runPerformanceTestImpl() {
		...
		postMessage('do-meta-ping', { 
			payload: testLog.payload, 
			times: testLog.times 
		});
	}
